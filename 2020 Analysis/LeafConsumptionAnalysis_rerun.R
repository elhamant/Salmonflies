...
library(car)
Area=read.csv("C:\\Users\\emily\\OneDrive\\Documents\\Undergrad\\Undergrad year 4\\Thesis\\Analysis\\Copy of clean.community.ecology.data.csv", header=T)
Area$leaf.area.eaten=replace(Area$leaf.area.eaten, Area$leaf.area.eaten==0, NA)
...
aggregate(Area$leaf.area.eaten, list(Area$treatment, Area$leaf.type), mean)


####testing for General differences of means####
table(Area$leaf.type)
...
par(mfrow=c(2,2))
plot(reg3)
tapply(X=Area$diff, INDEX= Area$leaf.type, FUN=sd)
car::leveneTest(aov(diff~leaf.type, data=Area))
TukeyHSD(reg3)
plot(TukeyHSD(reg3), las=2)
par(mar=c(4.2,9.5,3.8,2))


###testing for differences of means? by groups
